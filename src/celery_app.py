...
async def get_setup_utils():
    """
    Dependency initializer for asynchronous Celery workers.
    Sets up database sessions, vector stores, and LLM/Embedding client factories.
    """
    settings = get_settings()
...
# Configure Celery with robust production settings
celery_app.conf.update(
    ...
    # Connection settings for high reliability
    ...
    beat_schedule={
        "cleanup-old-task-records": {
            ...
            "schedule": 600,  # Run every 10 minutes (600s)
            "args": (),
        }
    },
